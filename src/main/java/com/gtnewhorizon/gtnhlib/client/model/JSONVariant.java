...
@Desugar
@Internal
public record JSONVariant(ModelLocation model, float x, float y, float z, boolean uvLock) implements BakeData {

    public JSONVariant(ModelLocation model, int x, int y, boolean uvLock) {
        this(model, x, y, 0, uvLock);
    }

    public JSONVariant(ModelLocation model, int x, int y, int z, boolean uvLock) {
    ...
    /// greater than zero.
    public static Weighted<JSONVariant> weightedVariant(ModelLocation model, int x, int y, int z, boolean uvLock,
            int weight) {
